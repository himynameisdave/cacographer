...
- **`bun run check` keeps its leading `svelte-kit sync`.** `svelte-check-rs` will run sync itself when `.svelte-kit/` is missing, but it is doing so as a convenience and its failures are sticky — a check that errors on a cold tree caches the bad program under `node_modules/.cache/svelte-check-rs/` and keeps reporting it until that directory is cleared. Syncing first costs ~0.2s and takes the ordering out of play. If `check` ever reports a phantom missing file, `rm -rf node_modules/.cache/svelte-check-rs` first.
